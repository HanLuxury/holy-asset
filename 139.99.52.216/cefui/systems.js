/* =====================================================================
   ...
   Pengganti tampilan textdraw untuk: notifikasi (ShowTDN), notify+icon
   (ShowNotify), item box (ShowItemBox).
   FOKUS TAMPILAN: dirender di dalam panggung virtual 640x448 pada
   POSISI yang sama dengan textdraw aslinya (skala mengikuti layar).
   ===================================================================== */
(function () {
  "use strict";
  ...
  // Posisi asli textdraw: x175,y350 (bar), label y338. Dirender di panggung.
  ...
  window.EAGLE.on("spawn", function (d) {
    ...
    spawnWrap.querySelectorAll(".esp-btn").forEach(function (el) {
      el.addEventListener("click", function () {
        ...
        window.EAGLE.emit("td_click", '{"s":"p","td":' + id + '}');
      });
    });
  });
