(() => {
  ...
  const el = {
    app: $('app'), boot: $('bootScreen'), serverName: $('serverName'), serverVersion: $('serverVersion'),
    topHud: $('topHud'), statusPanel: $('statusPanel'),
  ...
  function receiveFromPawn(event, payload) {
    ...
    switch (event) {
      ...
      case 'ui:login':
      case 'login:show':
      case 'hud:hide': return showLoginScreen(data);
      case 'ui:showHud':
      case 'hud:show':
      case 'login:success': return showGameHud(data);
      default:
  ...
  function bindNativeReceivers() {
    window.HolyCEF = { receive: receiveFromPawn, emit: sendToPawn, ready, showLoginScreen, showGameHud };
    ...
    if (window.cef && typeof window.cef.on === 'function') {
      ['cef:bootstrap', 'notification:push', 'gametext:show', 'hud:update', 'dialog:show', 'dialog:hide', 'ui:login', 'login:show', 'ui:showHud', 'hud:show', 'hud:hide', 'login:success'].forEach((name) => {
        try { window.cef.on(name, (payload) => receiveFromPawn(name, payload)); } catch (_) {}
      });
    }
  }

  function showLoginScreen(data = {}) {
    state.hudVisible = false;
    document.body.classList.remove('ui-game');
    document.body.classList.add('ui-login');
    el.boot.classList.remove('minimized');
    el.topHud.classList.add('hidden');
    el.statusPanel.classList.add('hidden');
    el.speedometer.classList.add('hidden');
    if (data.player) el.fakePlayer.value = data.player;
    if (data.status) el.fakeStatus.value = data.status;
  }

  function showGameHud(data = {}) {
    state.hudVisible = true;
    document.body.classList.remove('ui-login');
    document.body.classList.add('ui-game');
    el.boot.classList.add('minimized');
    el.topHud.classList.remove('hidden');
  ...
  function ready() {
    ...
    showLoginScreen({ status: 'Menunggu login server' });
    sendToPawn('cef:ready', { version: 'holy-fivem-ui-login-gated', time: Date.now() });
  }
  ...
  function onBootstrap(data) {
    ...
    el.fakeStatus.value = data.status || 'Server connected';

    if (Number(data.logged || data.loggedIn || data.showHud || data.inGame || 0) === 1) {
      showGameHud(data);
    } else {
      showLoginScreen(data);
    }
  }
  ...
  function updateHud(data) {
    if (!state.hudVisible) showGameHud(data);
