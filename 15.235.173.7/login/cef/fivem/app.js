(() => {
  ...
  function sendToPawn(event, payload = {}) {
    ...
    const candidates = [
      ...
      () => window.external && typeof window.external.invoke === 'function' && window.external.invoke(packed),
      ...
      () => typeof window.OnCefClientEvent === 'function' && window.OnCefClientEvent(event, json)
    ];
    for (const fn of candidates) {
      try { const r = fn(); if (r !== false && r !== undefined) return true; } catch (_) {}
    }
  ...
  function bindUiEvents() {
    if (el.dialogClose) el.dialogClose.onclick = () => submitDialog(0);
    if (el.readyButton) el.readyButton.onclick = () => { setStatus('Refresh dikirim ke server...'); ready(); };
  ...
  function ready() {
    ...
    sendToPawn('cef:ready', { version: 'holy-fivem-ui-loading-fixed', time: Date.now() });
    sendToPawn('ui:ready', { version: 'holy-fivem-ui-loading-fixed', time: Date.now() });
  }
  ...
  function renderList(info, hasHeader) {
    ...
    rows.forEach((line, index) => {
      ...
      row.addEventListener('click', () => selectRow(index, true)); list.appendChild(row);
    });
  ...
  function addActions(button1, button2) {
    if (button2) { const cancel = document.createElement('button'); cancel.className = 'dialog-btn danger'; cancel.textContent = cleanSampText(button2); cancel.onclick = () => submitDialog(0); el.dialogActions.appendChild(cancel); }
    const ok = document.createElement('button'); ok.className = 'dialog-btn primary'; ok.textContent = cleanSampText(button1 || 'OK'); ok.onclick = () => submitDialog(1); el.dialogActions.appendChild(ok);
  }
