(function(){
  ...
  function render(payload){
    ...
    weight.textContent=state.weight||'0/30 KG'; money.textContent='$'+(state.money||0).toLocaleString('id-ID'); level.textContent='Level '+(state.level||1);
    grid.innerHTML='';
    for(let i=0;i<(state.maxSlots||48);i++){
      ...
      if(it){ div.innerHTML=`<div class="model">OBJ ${esc(it.model)}</div><div class="count">x${esc(it.amount)}</div><div class="name">${esc(it.name)}</div>`; }
